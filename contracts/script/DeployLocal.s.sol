...
/// stand-in: a MockUSDC, the registry + bridge (deployer = well-known test
/// account #0, relayer = account #1), and mints some MockUSDC to the
/// deployer so it can immediately try `createChain`. NOT for any real
/// network — script/Deploy.s.sol is the real deploy path against a real
/// USDC.
///
/// The local geth L1 stand-in only funds one address at genesis
/// (TREASURY_ADDRESS on the `l1` compose service, set to this script's
/// deployer). So this script also forwards a slice of ETH to the
...
contract DeployLocal is Script {
    // The well-known Foundry/Hardhat test mnemonic's accounts #0, #1, #2.
