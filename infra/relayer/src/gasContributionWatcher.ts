...
/// ever powers a leaderboard, never anything that moves money, so a day of
/// staleness is completely fine (see index.ts, which calls this on a much
/// slower interval). Uses the same per-chain IndexerCursor pattern as every
...
export async function trackGasContributions(chain: ChainRow) {
  ...
  for (let blockNumber = fromBlock; blockNumber <= safeLatest; blockNumber++) {
    ...
    for (const txHash of block.transactions) {
      const receipt = await sideClient.getTransactionReceipt({ hash: txHash });
  ...
  if (spendByAddress.size > 0) {
    console.log(
      `[gas-contribution] chain ${chain.chainId}: updated ${spendByAddress.size} address(es) through block ${safeLatest}`
    );
  }
}
